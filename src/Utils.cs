...
namespace Microsoft.Azure.SignalR.Perf.Serverless
{
    public class Utils
    {
        ...
        public Utils(string connectionString)
        {
            (Endpoint, AccessKey, Port) = ParseConnectionString(connectionString);
        }
        ...
        internal static (string, string, string) ParseConnectionString(string connectionString)
        {
            ...
            if (properties.Length > 1)
            {
                ...
                {
                    var port = dict[PortProperty] ?? "443";
                    return (dict[EndpointProperty].TrimEnd('/'), dict[AccessKeyProperty], port);
                }
            }
